...
> App与H5交互函数的实现方法可以参考 [BusinessAPI/Client/README.md](../BusinessAPI/Client/README.md)
...
5. game_update_balance 游戏内/App发起更新余额（双向调用）
...
游戏内余额发生变化时，通知App：
```typescript
5. appJS.game_update_balance(score);
...
| code           | string | URL或JS函数 | 接入者提供的认证码（下面有说明） |
...
- URL参数解析，server_addr和score_icon参数传入时，需要先进行base64编码，再urlencode编码。（编码/解码规则详见[游戏开发流程 - 参数说明](./游戏开发流程.md)）
...
```
使用这种方式时，请注意：
1. **如果通过 URL 传递了必要的参数（包含user_id、server_addr和code），游戏加载完毕后将自动进入登录流程，不再等待 App 调用 game_start 函数**。否则，等待 App 调用 game_start 传参后才会进入用户认证、登录过程
2. **score_icon、score_icon_ext 和 language 三个参数总是使用 URL 传参的形式传递**
3. 其他参数推荐使用 JS 函数调用传参，能够更灵活的掌控游戏进程
...
##### App更新游戏内余额
当App内余额有变动需要更新到游戏中时，App通过调用游戏 JS 方法 game_update_balance 更新游戏内余额，如：
```typescript
gameJS.game_update_balance({
	user_id: '100001',
	score: 100
});
```
注意：
- 如果 user_id 参数有效（与游戏登录的用户ID相同），则通过游戏服务端调用 get_score 接口查询用户最新余额并更新到游戏客户端；（此种方式需控制调用频率）
- 否则使用 score 的值直接更新游戏内余额。
- 注意此函数是App主动调用游戏函数，更新游戏内的余额，需要跟上述游戏调用App函数更新App内余额的函数区分开（**appJS.game_update_balance(score)**）。
...
code相当于接入方用户的临时密码，与user_id对应，需要接入方自己生成和管理。

user_id和code通过客户端传给游戏H5客户端，游戏客户端会通过socket协议将user_id和code传递给游戏服务端，游戏服务端收到登录请求时，会调用接入方实现的get_sstoken接口，把这个code传递给接入方校验，确定是合法的用户登录游戏请求后生成sstoken，后续通过sstoken进行加减币操作。

虽然这个code机制完全由接入方实现，但出于安全考虑，有几点建议：
- code 设置有效期且为一次性有效
- sstoken 设置有效期，通过update_sstoken进行续期

一种可能的实现方案（仅供参考）：
- 用户在点击进入游戏时，先调用 api 接口获得游戏 code（例如：服务端可生成随机 code 保存在 Redis 中，key可以是用户ID，并设定 EXPIRE 为 1 分钟）
- 用户拿到 user_id 和 code 传递给游戏客户端，游戏服务端调用 get_sstoken 时进行校验（例如：通过 user_id 获取到 Redis 中的 code，如果匹配则通过，否则不通过）
- code 失效机制：
	1. 当通过 get_sstoken 校验 code 通过后，将 code 设为失效
	2. 当一定时间后（如1分钟），code 自动失效
...
注意：透传参数，是3OS要求实现的基本逻辑。如果游戏方实现了基于 user_data 的其他高级用法，需参考游戏开发者提供的用法说明，配合传参。
