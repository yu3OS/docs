...
- URL参数解析，server_addr和score_icon参数解析时，需要先进行base64解码，再urldecode解码。其他字符串参数仅需urldecode解码。(**注意：如果使用js传参数时URL不进行编码，直接获取URL即可**)
...
```
**game_update_balance函数属于双方用来通知金币的函数，游戏可以通过这个函数通知客户端金币的变换，客户端也可以在充值成功后通知游戏更新用户积分，需要客户端主动调用并且传递如下参数:**
