...
> App交互函数的实现方法可以参考 [这里](../BusinessAPI/Client/README)
...
当收到客户端登录请求，并得到客户端传递的必要参数后(通过socket协议)，服务端通过 [get_sstoken](../BusinessAPI/Server/get_sstoken) 接口做用户认证：
```go
//构造请求参数
req := SSTokenRequest{
	UserID: userId,
...
在一定的有效期内（一般为3天），可以通过 [update_sstoken](../BusinessAPI/Server/update_sstoken) 接口刷新sstoken，获得更长的有效期。

#### 获取用户信息

通过 [get_user_info](../BusinessAPI/Server/get_user_info) 接口，可以通过用户ID获取用户信息。

#### 获取用户积分

通过 [get_score](../BusinessAPI/Server/get_score) 接口，可以通过用户ID获取用户积分。

#### 用户加减分

通过 [change_score](../BusinessAPI/Server/change_score) 接口，可以对用户进行加减积分操作。
...
1. :3334/callback/health 健康检查接口 （[接口说明](../DevAPI/Health)）
2. :3334/callback/inform 通知接口 （[接口说明](../DevAPI/Inform)）
...
1. :3335/callback/config 配置接口 （[接口说明](../DevAPI/Config)）
2. :3335/callback/stat 数据查询接口 （[接口说明](../DevAPI/Stat)）
> 注意：PaaS要求游戏后台服务须总是监听在3335端口。
#### 回调接口

同样的，游戏后台服务也需要通过3335端口实现两个PaaS的回调接口：
1. :3335/callback/health 健康检查接口 （[接口说明](../DevAPI/Health)）
2. :3335/callback/inform 通知接口 （[接口说明](../DevAPI/Inform)）
> 注意：PaaS要求游戏后台服务须总是监听在3335端口。
