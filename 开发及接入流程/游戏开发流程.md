...
2.解析URL参数
```typescript
//本文所列代码均以cocos creator环境typescript语言（客户端）、Golang（服务端）为例，仅供参考。
...
- URL参数解析，server_addr和score_icon参数解析时，需要先进行urldecode解码，再base64解码。其他字符串参数仅需urldecode解码。(**注意：如果使用js传参数时URL不进行编码，直接获取URL即可**)
...
```typescript
on_app_called_game_update_balance(json:string) {
  //json例子:{user_id:10001,score:1000}
	//通过游戏协议，查询用户最新积分余额
	socket.send_get_balance(json.user_id);
}
...
//通过js函数告知接入方移动端最新积分余额
app.game_update_balance(score)
```
**game_update_balance函数属于双方用来通知积分的函数，游戏可以通过这个函数通知客户端积分的变换，客户端也可以在充值成功后通知游戏更新用户积分。**



5.移动端与游戏前端js交互实例(仅供参考)

```typescript
//用于与native交互（Android、iOS）
export class native {
    public static initJSFunctions() {
        if (!DEBUG) {
            if (native.getDeviceType() != 'iOS' && native.getDeviceType() != 'Android') {
                console.log('Wrong Device Type: ', native.getDeviceType());
                return;
            }
        }

        let _window = window;
        _window['game_update_balance'] = function(data:any){
            //let j = JSON.parse(data);
            if (util.mainObj) {
                util.mainObj.nativeUpdateBalance(+data.userId, +data.score);
            }
        }

        _window['game_start'] = function(data:any){
            //let j = JSON.parse(data);
            if (util.mainObj) {
                util.mainObj.nativeStartGame(data);
            }
        }
    }
    ...
    public static gameLoadProgress(progress:number) {
        try {
            let _window = window;
            switch(native.getDeviceType()) {
                case 'iOS':
                    _window["webkit"].messageHandlers.game_load_progress.postMessage(progress);
                    break;
                case 'Android':
                    _window["App"].game_load_progress(progress);
                    break;
                case 'Debug':
                    break;
                default:
            ...
            console.log('native loadProgress error:', e);
        }
    }

    public static gameLoadComplete(type:string) {
        try {
            let _window = window;
            switch(native.getDeviceType()) {
                case 'iOS':
                    _window["webkit"].messageHandlers.game_load_complete.postMessage(type);
                    break;
                case 'Android':
                    _window["App"].game_load_complete(type);
                    break;
                case 'Debug':
                    break;
                default:
            ...
            console.log('native loadComplete error:', e);
        }
    }

    public static gameDestroy() {
        try {
            let _window = window;
            switch(native.getDeviceType()) {
                case 'iOS':
                    _window["webkit"].messageHandlers.game_destroy.postMessage(null);
                    break;
                case 'Android':
                    _window["App"].game_destroy();
                    break;
                case 'Debug':
                    break;
                default:
            ...
            console.log('native gameDestroy error:', e);
        }
    }

    public static gameRecharge(type:number) {
        //type 1:余额不足提醒 2:跳转充值页面
        try {
            let _window = window;
            switch(native.getDeviceType()) {
                case 'iOS':
                    _window["webkit"].messageHandlers.game_recharge.postMessage(type);
                    break;
                case 'Android':
                    _window["App"].game_recharge(type);
                    break;
                case 'Debug':
                    break;
                default:
            ...
            console.log('native gameRecharge error:', e);
        }
    }

    public static updateGold(gold:number) {
        try {
            let _window = window;
            switch(native.getDeviceType()) {
                case 'iOS':
                    _window["webkit"].messageHandlers.game_update_balance.postMessage(gold);
                    break;
                case 'Android':
                    _window["App"].game_update_balance(gold);
                    break;
                case 'Debug':
                    break;
                default:
            ...
            console.log('native updateGold error:', e);
        }
    }
}
...
http.Post("api_paas_config", "application/json", bytes.NewBuffer(r))
...
user_data 是自定义参数（约定为json字符串），游戏可以使用user_data实现传递参数、匹配机制等高级用法。
游戏需要实现user_data从客户端到服务端接口的透传，在此基础上，可以用于其他高级用法。

基础用法（必须实现）：
1. 客户端传递参数给游戏，可对user_data进行参数设计，比如 {room_id:1000} 用于代表该用户打开游戏所在的直播间ID为1000
2. 游戏客户端通过登录协议，将 user_data 内容传递到PaaS，PaaS在调用用户认证接口 get_sstoken 时，将其传递给 App 服务端接口，再由 App 服务端根据这个参数，进行逻辑判断
3. get_sstoken 认证完成后，接口会返回另外一个 user_data，拿到这个 user_data ，在游戏调用 change_score 时传递给 App 服务端接口，做其他逻辑判断使用

高阶用法（可选）：
1. 匹配机制。游戏登录时，传递房间号、桌号，给游戏服务端，根据这些信息实现用户间的按桌匹配逻辑。
2. 参数加密。user_data 内携带的一些参数，App 可实现自己的加密逻辑，而游戏方不必关心，完成透传即可。

如果游戏实现了除参数透传以外的其他高级用法，需要在游戏的介绍中提供用法说明。
